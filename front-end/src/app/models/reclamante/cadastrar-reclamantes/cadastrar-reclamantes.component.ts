...
export class CadastrarReclamantesComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.erroPost) console.log('toerrado');
  }
  cadastrarReclamante(): void {
    ...
    if (this.reclamante.nome !== null || this.reclamante.nome !== undefined) {
      this.reclamanteService.cadastrarReclamante(this.reclamante).subscribe(
        () => {
          (this.sucessoPost = true), this.reclamanteForm.reset();
        },
        (err: HttpErrorResponse) => {
          this.erroPost = err.ok;
        }
      );
    }
  }
